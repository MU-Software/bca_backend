...
def is_profile_chat_invitable(
    ...
    if not target_profile:
        return ChatInvitableCheckReturnType(
            profile_id=-1, success=True, code=409, message='해당 프로필을 찾지 못했습니다.',
    ...
    if target_profile.uuid != requested_profile_id and target_profile.user_id == requested_user_id:
        return ChatInvitableCheckReturnType(
            profile_id=target_profile.uuid, success=True, code=409,
            message='해당 프로필은 다른 사람만이 초대할 수 있습니다.',
            data={'conflict_reason': ['해당 프로필은 다른 사람만이 초대할 수 있습니다.', ]})
    ...
    # Check is user trying to enter by themselves, or want to invite another user.
    if target_profile.uuid == requested_profile_id and target_profile.user_id == requested_user_id:
        # OK, User tries to enter the room.
        # There can be a two results.
        #   1. Requested user is not in target room, and target room is not private,
        #      so target user enters the room.
        #   2. Requested user is not in target room, and target room is private, so permission denied.
        # Let's check it out.
        ...
        # There can be a multiple results.
        ...
        #      2-1. and target user is not allowed to be invited by annonymous, so permission denied.
        #      2-2. elif target user is allowed to be invited by annonymous, then invitaion success.
        ...
        target_to_rel_rel = [p for p in rel_between_profiles
